...
docgen_read_input
docgen_selftest_dump "subagent_start"
...
case "$SUBAGENT_TYPE" in
  docgen-flow)
    CTX='docgen-flow 铁律（hook 注入）：1) 产物章节须齐全（TL;DR / 入口与触发 / 调用链总览 / 逐层解析 / 数据流转 / 外部依赖与副作用 / 错误与分支路径 / 改动风险 / 触达文件清单，断点章节按需）；2) 调用链每一跳必须带来源标注四选一（直接调用 / provider:接口→实现 / 推断:grep / ⚠️未能跟进:<形式>）；3) 严禁凭符号名臆测调用边——跟不下去就标断点 + grep 候选，绝不编造；4) 每跳内联代码 ≤20 行，超出标「… (+N 行)」；5) 元信息头 review 字段首版写「⏳ 待校验」，不要自填「✅ 通过」；6) 只写 docs/flows/ 下的目标文件。'
    ;;
  docgen-dir)
